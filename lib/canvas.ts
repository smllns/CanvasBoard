...
export const handleCanvasObjectModified = ({
  ...
  if (target.type === 'activeSelection' && target.canvas) {
    // If the target is an active selection and it's associated with a canvas
    const objects = target.getObjects();

    objects.forEach((obj) => {
      // For active selection, we need to continuously update the position as it's being dragged
      obj.on('moving', () => {
        syncShapeInStorage(obj);
      });
      // Sync the initial position
      syncShapeInStorage(obj);
    });
  } else {
    // For individual objects, sync their position when modified
    target.on('moving', () => {
...
export const getObjectById = ({
  ...
  fabricRef: fabric.Canvas | null; // Change the type to fabric.Canvas or null
  objectId: string;
}) => {
  if (!fabricRef) return null; // Check if fabricRef is null before accessing methods

  // Get all objects on the canvas
  const objects = fabricRef.getObjects();

  // Iterate over each object to find the one with the matching ID
